...
If a bus arrives at the station at a time tbus and a passenger arrived at a time tpassenger where 
...
as (
	...
	case when d.capacity < (d.total_passengers - finalcte.total_onboarded) then d.capacity else (d.total_passengers - finalcte.total_onboarded) end as onboarded_bus,
	finalcte.total_onboarded + case when d.capacity < (d.total_passengers - finalcte.total_onboarded) then d.capacity else (d.total_passengers - finalcte.total_onboarded) end as total_onboarded
	from finalcte 
	join cte d on d.rn = finalcte.rn+1
)
