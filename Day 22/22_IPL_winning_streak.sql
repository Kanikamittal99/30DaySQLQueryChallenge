...
teams_id_cte 
as(
	select dates,teams,result,row_number() over(partition by teams order by teams,dates) as rn
	from teams_cte as tc 
	join ipl_results as ir on tc.teams=ir.away_team or tc.teams=ir.home_team
)
,teams_diff_cte 
as(
	select *, rn - ROW_NUMBER() over(partition by teams order by teams,dates) as diff
	from teams_id_cte
	where result = teams
),
teams_final_cte 
as(
	select teams, COUNT(diff) oveR(partition by teams,diff order by teams,dates) as cnt
	from teams_diff_cte
)
select tc.teams,isnull(MAX(cnt),0) as max_winning_streak
from teams_cte as tc left join teams_final_cte as tfc on tc.teams=tfc.teams
group by tc.teams
