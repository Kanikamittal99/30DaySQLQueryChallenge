...
With cte as
(
select country, age, ROW_NUMBER() over(partition by country order by age) as rn,
cast(count(id) over(partition by country order by age range between unbounded preceding and unbounded following) as float)as cnt
from people
)
select country, age
from cte
