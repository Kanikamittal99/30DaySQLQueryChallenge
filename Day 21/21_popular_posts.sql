...
select us.session_id,
		pv.post_id,
		DATEDIFF(SECOND,session_starttime,session_endtime) as session_duration,
		pv.perc_viewed,
		DATEDIFF(SECOND,session_starttime,session_endtime) * pv.perc_viewed/100 as view_time_per_session_post
from post_views pv left join user_sessions us on pv.session_id=us.session_id


SELECT * 
FROM (
	SELECT pv.post_id,
			SUM(DATEDIFF(SECOND,session_starttime,session_endtime) * pv.perc_viewed/100) AS total_viewtime
	FROM post_views pv LEFT JOIN user_sessions us ON pv.session_id=us.session_id
	GROUP BY pv.post_id 
) sq
WHERE total_viewtime > 5